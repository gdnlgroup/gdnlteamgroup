...
/**
 * Handles the add projet shortcode
 *
 * @author Tareq Hasan
 * @package WP User Frontend
 * @param $atts
 */
function wpuf_add_message_shorcode( $atts ) {

    extract( shortcode_atts( array('post_type' => 'message'), $atts ) );

    ob_start();

    if ( is_user_logged_in() ) {
        wpuf_add_message( $post_type );
    } else {
       Redirect_('login');
        wp_die();
...
add_shortcode( 'wpuf_addmessage', 'wpuf_add_message_shorcode' );
        ...
        <div id="wpuf-post-area">
            <form id="wpuf_new_post_form" name="wpuf_new_post_form" action="" enctype="multipart/form-data" method="POST">
               <input type="hidden" value="<?php echo $userdata->ID;?>"  />
                <?php wp_nonce_field( 'wpuf-add-projet' ) ?>
                <select>
                <?php
                ...
                <ul class="wpuf-post-form">
                    ...
                    <li>
                        ...
                        <div class="clear"></div>
                        <p class="description">
                             <?php echo stripslashes( get_option( 'wpuf_title_help' ) ); ?></p>
                    </li>
                    ...
                    <li>
                        ...
                        <div class="clear"></div>
                        <p class="description">
                        <?php echo stripslashes( get_option( 'wpuf_desc_help' ) ); ?>
                        </p>
                    </li>
...
function wpuf_init_message_check() {
    if ( has_shortcode( 'wpuf_addprojet' ) ) {
        if ( isset( $_POST['wpuf_post_new_submit'] ) ) {
            $nonce = $_REQUEST['_wpnonce'];
            if ( !wp_verify_nonce( $nonce, 'wpuf-add-projet' ) ) {
                wp_die( __( 'Cheating?' ) );
            }

            wpuf_validate_projet_submit();
        }
    }
}

add_action( 'template_redirect', 'wpuf_init_projet_check' );
 ...
 */
function wpuf_validate_message_submit() {
    global $userdata;
