 ...
 * @param $atts
 */
function wpuf_add_pending_shorcode( $atts ) {
...
add_shortcode( 'wpuf_pendingpost', 'wpuf_add_pending_shorcode' );
...
function wpuf_init_pending_check() {
    if ( has_shortcode( 'wpuf_pendingpost' ) ) {
        if ( isset( $_POST['wpuf_post_new_submit'] ) ) {
            ...
            wpuf_validate_pending_submit();
        }
    }
}

add_action( 'template_redirect', 'wpuf_init_pending_check' );
