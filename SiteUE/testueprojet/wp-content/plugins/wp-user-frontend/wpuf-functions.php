...
function wpuf_show_post_status( $status ) {
    ...
    if ( $status == 'publish' ) {
        ...
        $title = __( 'Attente Validation', 'wpuf' );
        $fontcolor = '#C00202';
    } else if ( $status == 'future' ) {
        $title = __( 'planifié', 'wpuf' );
        $fontcolor = '#bbbbbb';
    }
...
function wpuf_enqueue_scripts() {
    ...
    if ( has_shortcode( 'wpuf_pendingpost' ) || has_shortcode( 'wpuf_addprojet' ) || has_shortcode( 'wpuf_addpost' ) || has_shortcode( 'wpuf_edit' ) || has_shortcode( 'wpuf_dashboard' ) ) {
