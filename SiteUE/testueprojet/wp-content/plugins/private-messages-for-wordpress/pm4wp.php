...
//$role = $current_user->roles[0];
		...
		//	$error = true;
		//	$status[] = __( utf8_encode('Vous avez exceder la limite de votre boite à reception. Veuillez supprimer vos messages avant d\'envoyer un nouveau message.'), 'pm4wp' );
		//}
		...
		$recipient=explode(',','cherbonneau');
		$recipient=explode(',','admin');
		...
		if ( empty( $recipient ) ) {
			$error = true;
			$status[] = __( 'Veuillez entrer un nom d\'utilisateur correct.', 'pm4wp' );
		}
		if ( empty( $subject ) ) {
			$error = true;
			$status[] = __( 'Veuillez entrer l\'objet du message.', 'pm4wp' );
		}
		
		if ( empty( $nom ) ) {
			$error = true;
			$status[] = __( 'Veuillez entrer votre nom', 'pm4wp' );
		}
		if ( empty( $prenom ) ) {
			$error = true;
			$status[] = __( 'Veuillez entrer votre pr&eacute;nom', 'pm4wp' );
		}
		if ( empty( $email ) ) {
			$error = true;
			$status[] = __( 'Veuillez entrer votre email', 'pm4wp' );
		}
		 

		if ( !is_email( $email ) ) {
			$error = true;
			$status[] = __( 'Veuillez entrer un  email valide', 'pm4wp' );
 			$user_email = '';
		}
		...
		*/
		...
		if ( !$error ) {
			...
			$f="La personne suivante demande &agrave; s\'incrire comme encadrant<br>";
			$f.='Nom: ' .$nom.'<br>';
			$f.='Prenom: ' .$prenom.'<br>';
			$f.='Email: ' .$email.'<br>';
			$f.='Autres supplementaires: <br>';
			$f.=$content;
			$content=$f;
			$numOK = $numError = 0;
		...
		echo '<div id="message" class="updated fade" style="margin-left: 10%;" ><p>', implode( '</p><p>', $status ), '</p></div>';
	}
	?>
<form method="post" action="" id="send-form" >
	...
	<?php 
		...
		//if ( $option['type'] == 'autosuggest' ) { // if auto suggest feature is turned on
			?>
			<input type="hidden" id="recipient" value='cherbonneau' />
		...
		<tr>
			...
			<td><input type="hidden" name="subject" value="Demande Isnscription"/></td>
		</tr>
		<tr>
			<th><?php _e( '<strong>Votre nom<strong>', 'pm4wp' ); ?></th>
			<td><input type="text" name="nom" size="15" /></td>
		</tr>
		
		<tr>
			<th><?php _e( '<strong>Votre Pr&eacute;nom<strong>', 'pm4wp' ); ?></th>
			<td><input type="text" name="prenom" size="15"/></td>
		</tr>
		
		<tr>
			<th><?php _e( '<strong>Votre Email<strong>', 'pm4wp' ); ?></th>
			<td><input type="text" name="email" size="15"/></td>
		</tr>
		
		<tr>
			<th><?php _e( '<strong>Autres informations</strong>', 'pm4wp' ); ?></th>
			<td >
			<textarea name="content" id="content"  style=" width:450px; height: 150px;"></textarea>
 			</td>
			 
		</tr>
	</table>
	<p class="submit" id="submit" style="margin-left: 40%;">
 ...
 * Send form page
 */
add_shortcode( 'ecrire_m', 'rwpm_send' );

function rwpm_send( ) {
	global $wpdb, $current_user;
	?>
<div class="wrap">
<h2><?php _e( '<span style="margin-left:10%;">Envoyer un message</span>', 'pm4wp' ); ?></h2>
	<?php
	$option = get_option( 'rwpm_option' );
	if ( $_REQUEST['page'] == 'rwpm_send' && isset( $_POST['submit'] ) ) {
		$error = false;
		...
		if ( empty( $content ) ) {
			$error = true;
			$status[] = __( 'Veuillez entrer le contenu du message', 'pm4wp' );
		}
...
<form method="post" action="" id="send-form">
	<table>
		<tr>
			<th  valign="top"><?php _e( '<strong>Destinateur</strong>', 'pm4wp' ); ?></th>
			<td>&nbsp;
	<?php 
 				// if message is not sent (by errors) or in case of replying, all input are saved
		...
		if ( $option['type'] == 'autosuggest' ) { // if auto suggest feature is turned on
			...
			?>
			<input  id="recipient"  <?php  if(!empty($_GET['recipient'])) echo 'value="'.$_GET["recipient"] .'"' ; ?>  />
		<?php if(empty($_GET['recipient'])) {  ?>
			...
			<?php } unset($_GET['recipient']) ;?>
							<?php

		} else { // classic way: select recipient from dropdown list
			?>
			<select name="recipient[]" multiple="multiple" size=10">
				<?php
				foreach ( $users as $user ) {
					$ln=get_usermeta($user->ID,'last_name',true);
					$fn=get_usermeta($user->ID,'first_name',true);
				$selected = ( $user->display_name == $recipient ) ? ' selected="selected"' : '';
				echo "<option value='$user->display_name'$selected>$user->display_name $fn $ln</option>";
			}
				?>
			</select>
				<?php
		...
		<tr>
			<th><?php _e( '<strong>Objet<strong>', 'pm4wp' ); ?></th>
			<td><input type="text" name="subject" value="<?php echo $subject; ?>"/></td>
		</tr>
		<tr>
			<th><?php _e( '<strong>Contenu</strong>', 'pm4wp' ); ?></th>
			<td>
		   <?php wp_editor( $content, 'content', array('textarea_name' => 'content', 'teeny' => true, 'media_buttons'=>false,'textarea_rows' => 8) ); ?>

			 
		</tr>
	</table>
	<p class="submit" id="submit" style="margin-left: 60%;">
		<input type="hidden" name="page" value="rwpm_send"/>
		<input type="submit" name="submit" class="button-primary" value="<?php _e( 'Send', 'pm4wp' ) ?>"/>
	</p>
...
function rwpm_inbox( ) {
	...
	if ( isset( $_GET['action'] ) && 'view' == $_GET['action'] && !empty( $_GET['id'] ) ) {
		...
		?>
	<div class="wrap">
		<h2><strong><?php _e( 'Inbox \ View Message', 'pm4wp' ); ?></strong></h2>

		<p><a href="?page=rwpm_inbox"><?php _e( 'Retour aux messages', 'pm4wp' ); ?></a></p>
		<table >
			<thead>
			<tr class="dsh">
				<th class="manage-column" width="20%"><?php _e( 'Info', 'pm4wp' ); ?></th>
				 <th class="manage-column"><strong><?php _e( 'Message', 'pm4wp' ); ?></strong></th> 
				<th class="manage-column" width="15%"><strong><?php _e( 'Action', 'pm4wp' ); ?></strong></th>
			</tr>
			</thead>
			<tbody>
			<tr class="dsh">
				<td class="dsh"><?php printf( __( '<b>Sender</b>: %s<br /><b>Date</b>: %s', 'pm4wp' ), $msg->sender, $msg->date ); ?></td>
				<td class="dsh"><?php printf( __( '<p><b>Subject</b>: %s</p><p>%s</p>', 'pm4wp' ), stripcslashes( $msg->subject ), nl2br( stripcslashes( $msg->content ) ) ); ?></td>
				<td class="dsh">
						<span class="delete"><small>
	...
	?>
<div class="wrap">
	<h2><strong><?php _e( 'Inbox', 'pm4wp' ); ?></strong></h2>
	<?php
	if ( !empty( $status ) ) {
		...
		<form action="" method="get">
			...
			<table class="widefat fixed" cellspacing="0">
				...
				<tr>
					...
					<th class="manage-column" width="10%"><strong><?php _e( 'Sender', 'pm4wp' ); ?></strong></th>
					<th class="manage-column"><strong><?php _e( 'Subject', 'pm4wp' ); ?></strong></th>
					<th class="manage-column" width="20%"><strong><?php _e( 'Date', 'pm4wp' ); ?></strong></th>
				</tr>
				</thead>
				<tbody>
					<?php
				...
				<tr>
					...
					<td class="dsh"><?php echo $msg->sender; ?></td>
					<td class="dsh">
						<?php
						if ( $msg->read ) {
						echo '<span style=" padding-left:2%;"><small><a href="', wp_nonce_url( "?page=rwpm_inbox&action=view&id=$msg->id", 'rwpm-view_inbox_msg_' . $msg->id ), '">', stripcslashes( $msg->subject ), '</a></small> </span>';
					} else {
						echo '<span style=" padding-left:2%;"><small><a href="', wp_nonce_url( "?page=rwpm_inbox&action=view&id=$msg->id", 'rwpm-view_inbox_msg_' . $msg->id ), '"><b>', stripcslashes( $msg->subject ), '</b></a></small></span>';
					}
						?>
					...
					<td class="dsh"><?php echo $msg->date; ?></td>
				</tr>
						<?php
				...
				<tr>
					<th class="manage-column check-column"><input type="checkbox"/></th>
					<th class="manage-column"><strong><?php _e( 'Sender', 'pm4wp' ); ?></strong></th>
					<th class="manage-column"><strong><?php _e( 'Subject', 'pm4wp' ); ?></strong></th>
					<th class="manage-column"><strong><?php _e( 'Date', 'pm4wp' ); ?></strong></th>
				</tr>
				</tfoot>
			</table>
		</form>
...
function rwpm_outbox( ) {
	...
	if ( isset( $_GET['action'] ) && 'view' == $_GET['action'] && !empty( $_GET['id'] ) ) {
		...
		?>
	<div class="wrap">
		<h2><strong><?php _e( 'Outbox \ View Message', 'pm4wp' ); ?></strong></h2>
  ...
  // don't need to do more!
	...
	?>
<div class="wrap">
	<h2><strong><?php _e( 'Outbox', 'pm4wp' ); ?></strong></h2>
	<?php
	if ( !empty( $status ) ) {
	echo '<div id="message" class="updated fade"><p>', $status, '</p></div>';
}
	if ( empty( $msgs ) ) {
		...
		<form action="" method="get">
			...
			<div class="tablenav">
				<input type="submit" class="button-secondary" value="<?php _e( 'Supprimer message', 'pm4wp' ); ?>"/>
			</div>
			...
			<table class="dsh">
				...
				<tr class="dsh">
					...
					<th  class="manage-column width="10%"> <strong><?php _e( 'Recipient', 'pm4wp' ); ?></strong></th>
					<th class="manage-column"><strong><?php _e( 'Subject', 'pm4wp' ); ?></strong></th>
					<th class="manage-column" width="20%"><strong><?php _e( 'Date', 'pm4wp' ); ?></strong></th>
				</tr>
				</thead>
				...
				<tr class="dsh">
					<th class="check-column"><input type="checkbox" name="id[]" value="<?php echo $msg->id; ?>"/></th>
					<td class="dsh"><?php echo $msg->recipient; ?></td>
					<td class="dsh" >
						<?php
						echo '<span style="padding-left:2%;"><a href="', wp_nonce_url( "?page=rwpm_outbox&action=view&id=$msg->id", 'rwpm-view_outbox_msg_' . $msg->id ), '">', stripcslashes( $msg->subject ), '</a></span>';
						?>
						<div class="row-actions" style="text-align: right;">
							<span >
				...
				<tr>
					<th class="manage-column check-column"><input type="checkbox"/></th>
					<th class="manage-column"><strong><?php _e( 'Recipient', 'pm4wp' ); ?></strong></th>
					<th class="manage-column"><strong><?php _e( 'Subject', 'pm4wp' ); ?></strong></th>
					<th class="manage-column"><strong><?php _e( 'Date', 'pm4wp' ); ?></strong></th>
				</tr>
				</tfoot>
