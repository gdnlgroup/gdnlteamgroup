...
function moncustomposttype_init()
{
  ...
  register_post_type($myposttype,$args);
	...
	register_taxonomy( 'modules', $myposttype, array( 'hierarchical' => true, 'label' => 'modules', 'query_var' => true, 'rewrite' => true ) ); 
}	
...
function moncustomposttype_form(){
	...
	if((isset($_GET['post_type'])) and ($_GET['post_type']==$myposttype)) /* formulaire d'ajout */
	{
		...
		foreach ($myCustomTypeOptions as $o)
		{
			...
			echo '<h3 class="handle"><span>'.$o['name'].'</span></h3><div class="inside"><label class="screen-reader-text" for="'.$o['id'].'">'.$o['name'].'</label>';
